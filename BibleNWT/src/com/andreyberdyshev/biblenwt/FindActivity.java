...
public class FindActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Toast.makeText(getApplicationContext(), "Пустая строка!",
		...
		if (!aps.getMapData().isEmpty()) {
			...
			SimpleAdapter sAdapter = new SimpleAdapter(getApplicationContext(),
					aps.getMapData(), R.layout.show_find_result, from, to);
			FindList.setAdapter(sAdapter);

		}

		FindButton = (Button) findViewById(R.id.buttonGoFind);
		// хандлер срабатывает после завершения поиска в потоке, вызываемого по
		// нажатию кнопки
		h = new Handler() {
			public void handleMessage(android.os.Message msg) {

				FindActivity.this.setTitle(getResources().getString(
						R.string.title_reslut_find_window)
						+ DataCount);
				String[] from = { "title", "stix" };
				int[] to = { R.id.TitleField, R.id.textStixField };

				SimpleAdapter sAdapter = new SimpleAdapter(
						getApplicationContext(), aps.getMapData(),
						R.layout.show_find_result, from, to);
				FindList.setAdapter(sAdapter);
					...
					for (int i = 1; i < CountRec + 1; ++i) {
						...
						if (StrFind.indexOf(fText) > 0) {
							...
							String Titles1 = c.getString(ColBook) + " "
									+ c.getString(ColChapter) + ":"
									+ c.getString(ColNumStix);
							Titles.add(Titles1);
							Chapters.add(StrFind);

							// Toast.makeText(getApplicationContext(),
							// "Найдено: "+Titles1, Toast.LENGTH_SHORT).show();
